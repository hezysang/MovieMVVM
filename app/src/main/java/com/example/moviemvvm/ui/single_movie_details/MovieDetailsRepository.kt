...
class MovieDetailsRepository(private val apiService: TheMovieDBInterface) {
    ...
    fun fetchMovieSingleDetails(compositeDisposable: CompositeDisposable, movieId: Int): LiveData<MovieDetails>{

        movieDetailsNetworkDataSource = MovieDetailsNetworkDataSource(apiService, compositeDisposable)
        movieDetailsNetworkDataSource.fetchMovieDetails(movieId)

        return movieDetailsNetworkDataSource._downloadedMovieResponse
    }

    fun getMovieDetailsNetworkDState(): LiveData<NetworkState> {
